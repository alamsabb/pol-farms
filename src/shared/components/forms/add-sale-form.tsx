...
export function AddSaleForm({ onSubmit, onCancel, isLoading }: AddSaleFormProps) {
  ...
  const selectedBatch = batches.find(b => b._id === selectedBatchId && b.status === 'active' && b.currentBirdCount > 0)
    ...
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
        ...
        <Controller
          ...
          render={({ field }) => (
            <Select value={field.value} onValueChange={field.onChange}>
              ...
              <SelectContent>
                {batches.filter(b => b.status === 'active' && b.currentBirdCount > 0).map((batch) => (
      ...
      <div>
        ...
        <Input
          ...
          placeholder={selectedBatch ? `Max: ${selectedBatch.currentBirdCount}` : 'Select batch first'}
          disabled={!selectedBatch}
          className={errors.birdsSold || customErrors.birdsSold ? 'border-red-500' : ''}
        />
      ...
      <div className="flex justify-end space-x-3 pt-4">
        ...
        <Button type="submit" disabled={isLoading || !selectedBatch || Object.keys(errors).length > 0 || Object.keys(customErrors).length > 0}>
